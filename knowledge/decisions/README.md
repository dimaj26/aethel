...
- [ADR 0011 — De-duplicate session mechanics out of the core kernel](0011-dedupe-session-mechanics-from-core.md) — the verbose Route B session prose moves to the already-indexed session-lifecycle topic (single source of truth); kernel keeps happy-path + verbs + two cautions; `CORE-REV` 10→11.
