...
* **This is not an optional route.** Whenever a Route A or Route B change touches the items above, you MUST complete Route C in the SAME commit, BEFORE finishing. The pre-commit linter enforces this: its `[sync]` drift check flags a commit that stages code without updating a spec file (`memory.json`, `CONTEXT.md`, or `AETHEL.md`).
* **Protocol**:
