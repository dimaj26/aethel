...
def check_plan_stage():
    """Validates implementation_plan.md and task.md format."""
...
def check_memory_integrity():
    """Validates memory.json graph integrity."""
    ...
    mandatory_nodes = ["Aethel", "MemoryMCP"]
    for node in mandatory_nodes:
        if node not in entities:
            print_error(f"Mandatory core entity '{node}' is missing from the knowledge graph.")
            has_errors = True
    ...
    # We construct an adjacency list and run DFS to detect cycles.
    adj = {name: [] for name in entities}
    for rel in relations:
        # Avoid key error if nodes were broken/missing
        if rel["from"] in adj:
